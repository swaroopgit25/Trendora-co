...
Fiamma has external login support using GitHub.
...
Initially, we had hoped to show a platform with OpenID Connect and with PKCE. However, GitHub had neither of these features at the time the code was added to Fiamma. We really wanted to use an account platform that we knew devs would use, so that's why we stayed with GitHub.
